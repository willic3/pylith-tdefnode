...
class CreateTdefnodeQuads(Application):
    ...
    nodFile = pyre.inventory.str("nod_file", default="faults.nod")
    nodFile.meta['tip'] = "The name of the .nod file to read."
    
    extendFile = pyre.inventory.str("extend_file", default="extend.txt")
    extendFile.meta['tip'] = "File with the extension amount for each fault."

    fileType = pyre.inventory.str("file_type", default="tdefnode",
                                  validator = pyre.inventory.choice(["defnode", "tdefnode"]))
    fileType.meta['tip'] = "DEFNODE or TDEFNODE .nod file."

    nearSurfEps = pyre.inventory.dimensional("near_surf_eps",
                                             default=100.0*m)
    ...
    outputRotation = pyre.inventory.dimensional("output_rotation", default=45.0*degree)
    ...
    def _configure(self):
        ...
        cosR = math.cos(rot)
        sinR = math.sin(rot)
        self.rotMat = numpy.array([[ cosR, -sinR, 0.0],
                                   [ sinR,  cosR, 0.0],
    ...
    def _processFault(self, lines):
        ...
        xIndex = numpy.zeros(numNodes, dtype=numpy.int)
        zIndex = numpy.zeros(numNodes, dtype=numpy.int)
        lon = numpy.zeros(numNodes, dtype=numpy.float64)
        lat = numpy.zeros(numNodes, dtype=numpy.float64)
        elev = numpy.zeros(numNodes, dtype=numpy.float64)
        ...
        coords = numpy.column_stack((x, y, z))
        coordsRot = numpy.dot(self.rotMat, coords.transpose()).transpose()
        ...
        vertInds = numpy.arange(1, numNodesExt + 1, step=1, dtype=numpy.int32).reshape((numAsNodesExt, numDdNodesExt))
    ...
    def _writeSurfJournal(self, surfJournal, vertInds, numAsNodes, numDdNodes):
        ...
        cNum = 0
        curveInds = numpy.zeros((numAsNodes -1, numDdNodes - 1, 4), dtype=numpy.int32)
    ...
    def _createExtCoords(self, coords, numAsNodes, numDdNodes, upExt, downExt, asNegExt, asPosExt):
        ...
        zMax = numpy.amax(coords[:,2])
        numAsNodesExt = numAsNodes + 2
        numDdNodesExt = numDdNodes + 2
        topNode = 1
        coordsReshape = coords.reshape((numAsNodes, numDdNodes, 3), order='F')
        coordsExt = numpy.zeros((numAsNodesExt, numDdNodesExt, 3))

        # Extend updip, if possible.
        top = coordsReshape[:,0,:]
        atSurface = numpy.allclose(top[:,2], 0.0, atol=self.nearSurfEps.value)
    ...
    def _writeVtk(self, faultName, coords, numNodes, numAsNodes, numDdNodes):
        ...
        v.write(headConnect)
        numpy.savetxt(v, connect, fmt=intFmt)
        v.write(headCellType)
        numpy.savetxt(v,cellType, fmt=intFmt)
