...
outSpatialDb = 'merged_velmodel2.spatialdb'
outOrig1VTK = 'original_velmodel1.vtk'
outOrig2VTK = 'original_velmodel2.vtk'
outMergedVTK = 'merged_velmodel2.vtk'
outPremVTK = 'prem_velmodel2.vtk'
...
def readVelMod2(inFile):
    ...
    depths = -1000.0*numpy.array(depths)

    # (zz, yy, xx) = numpy.meshgrid(depths, lats, lons, indexing='ij')
    coordsGeog = numpy.zeros((numPoints, 3), dtype=numpy.float64)

    vp = numpy.zeros(numPoints, dtype=numpy.float64)
    vs = numpy.zeros(numPoints, dtype=numpy.float64)
    ...
    sortInds = numpy.lexsort((coordsGeog[:,0], coordsGeog[:,1], coordsGeog[:,2]))
...
(xCart2, yCart2, zCart2) = transform(projWGS84, projTM, vCoordsGeog2[:,0], vCoordsGeog2[:,1], vCoordsGeog2[:,2])
coordsCart2 = numpy.column_stack((xCart2, yCart2, zCart2))
num2VX = vLons2.shape[0]
num2VY = vLats2.shape[0]
num2VZ = vElevs2.shape[0]

print("Reading original velocity model:")
vDat = numpy.loadtxt(velModelFile1, dtype=numpy.float64)
vDat[:,2] *= -1000.0
...
for layer in range(3):
    vDatExt = numpy.vstack((vDatTop[:num1VX*num1VY,:], vDatExt))
...
(xCart1, yCart1, zCart1) = transform(projWGS84, projTM, vCoordsGeog1[:,0], vCoordsGeog1[:,1], vCoordsGeog1[:,2])
coordsCart1 = numpy.column_stack((xCart1, yCart1, zCart1))

# Get indices of old velocity model to use for interpolation.
xInner1 = numpy.logical_and(vCoordsGeog1[:,0] >= vLons2[0], vCoordsGeog1[:,0] <= vLons2[-1])
yInner1 = numpy.logical_and(vCoordsGeog1[:,1] >= vLats2[0], vCoordsGeog1[:,1] <= vLats2[-1])
zInner1 = numpy.logical_and(vCoordsGeog1[:,2] >= vElevs2[0], vCoordsGeog1[:,2] <= vElevs2[-1])
indsInside1 = xInner1*yInner1*zInner1
indsOutside1 = numpy.logical_not(indsInside1)
...
lonsInner = numpy.linspace(vLons1[0], vLons1[-1], num=numLonsInner, dtype=numpy.float64)
numLatsInner = int((vLats1[-1] - vLats1[0])/dLat) + 1
latsInner = numpy.linspace(vLats1[0], vLats1[-1], num=numLatsInner, dtype=numpy.float64)
numElevsInner = int((vElevs1[-1] - vElevs1[0])/dElev) + 1
elevsInner = numpy.linspace(vElevs1[0], vElevs1[-1], num=numElevsInner, dtype=numpy.float64)
...
# New ranges.
lonsNew = numpy.concatenate((westLonsAdd, lonsInner[1:-1], eastLonsAdd))
latsNew = numpy.concatenate((southLatsAdd, latsInner[1:-1], northLatsAdd))
elevsNew = numpy.concatenate((bottomElevsAdd, elevsInner[1:]))
...
xOrig = numpy.logical_and(coordsNewGeog[:,0] >= vLons1[0], coordsNewGeog[:,0] <= vLons1[-1])
yOrig = numpy.logical_and(coordsNewGeog[:,1] >= vLats1[0], coordsNewGeog[:,1] <= vLats1[-1])
...
refCoords = numpy.vstack((coordsCart2, coordsUse1, coordsOuter))
refVp = numpy.concatenate((vVp2, vVpUse1, vpOuter))
refVs = numpy.concatenate((vVs2, vVsUse1, vsOuter))
refDensity = numpy.concatenate((vDensity2, vDensityUse1, densityOuter))
